...
├── favicon.ico                     → Ícone da aba do navegador (formato clássico)
├── css/
...
├── assets/
│   └── favicon/                    → ícone da aba em outros formatos/tamanhos (SVG, PNG, manifest)
...
- **Ícone da aba do navegador (favicon)** baseado no mesmo selo circular "IL"
  usado no logo do menu — ver seção dedicada logo abaixo

## Sobre o favicon (ícone da aba)

O ícone foi desenhado a partir do mesmo círculo vermelho com as iniciais "IL"
usado no `.logo-badge` do menu, para manter a marca consistente até no ícone
da aba do navegador.

- `favicon.svg` — versão vetorial (nitidez perfeita em qualquer tamanho/tela),
  usada por navegadores modernos.
- `favicon.ico`, `favicon-16x16.png`, `favicon-32x32.png` — versões em bitmap
  para navegadores mais antigos.
- `apple-touch-icon.png` — ícone exibido quando alguém adiciona o site à
  tela inicial de um iPhone/iPad.
- `android-chrome-192x192.png` / `-512x512.png` + `site.webmanifest` — ícones
  usados no Android e por navegadores que suportam instalação do site como
  app (PWA).

Para trocar o ícone no futuro (ex.: usar a logo oficial em alta resolução),
edite `assets/favicon/favicon.svg` — ou gere um novo conjunto de imagens a
partir de um logo em PNG usando um gerador de favicon online (ex.:
realfavicongenerator.net) e substitua os arquivos dentro de `assets/favicon/`,
